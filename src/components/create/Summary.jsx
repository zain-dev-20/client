...
const Summary = () => {
  ...
  const handleSubmit = async(e) => {
    ...
    if(form) await updateForm({id:form._id,data:payload})
      else await createForm(payload)
    dispatch(resetForm())
    navigate("/")
    // if(form) return console.log("state here >>>>>>>>>>>",form._id,payload);
        ...
        <div key={idx} className='gap-4'>
          ...
          {Object.entries(currentState).map(([key,value], index) => (
            <div key={index} className='flex items-center gap-2 mb-2 rounded-xl px-2'>
              ...
              <p className="shadow text-gray-700 text-md bg-stone-100 py-2 px-4 w-1/2 rounded-xl truncate">{formatedValue(key,value)}</p>
            </div>
          ))}
        </div>
      );
